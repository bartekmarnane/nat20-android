    ...
    Scaffold(
        ...
        Column(Modifier.padding(inner).fillMaxSize()) {
            Column(Modifier.weight(1f).fillMaxWidth().verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                when (step) {
                    PfWiz.NAME -> OutlinedTextField(name, { name = it }, label = { Text("Character name") }, singleLine = true, keyboardOptions = KeyboardOptions(capitalization = KeyboardCapitalization.Words), modifier = Modifier.fillMaxWidth())
                    PfWiz.ANCESTRY -> {
