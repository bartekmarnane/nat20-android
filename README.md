...
- [ ] **A1. Project skeleton** — Android Studio project, Gradle (Kotlin DSL) setup, git repo + `.gitignore`, module structure (`:app`, `:domain`, `:ruleset-dnd5e`, `:data` proposed — mirrors the iOS package split so the domain stays ruleset-agnostic and the 5e engine is isolated).
- [ ] **A2. Core domain abstractions** — `Ruleset`, `Character`, `CharacterPayload`, `CharacterEvent`, `CharacterIntent` as Kotlin interfaces + value types (data classes / sealed classes). Intent-logged mutations via `Character.apply`. First unit tests (JUnit5 / Kotest) covering creation, rename, validation failures. `kotlinx.serialization` for the Codable-equivalent codec surface.
- [ ] **A3. D&D 5e ruleset, minimal slice** — `DnD5eRuleset` + `DnD5ePayload` (race, class, level, six abilities, max HP). `Ability` enum + ability-score modifier math. The first handful of intents. Tests covering ruleset, payload defaults, proficiency-bonus-by-level, modifier math, each intent, and a full mini character build.
