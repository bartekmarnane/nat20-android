...
- [~] **A10. Expand 5e content** *(taken in slices)* — **Done — Spell Library:** `Spell` model decoding the raw 5e-database SRD JSON (319 spells) via `DnD5eCatalog.spellLibrary`; `SpellLibraryScreen` (search + level filter + expandable rows) reachable from **Settings → Reference** and the codex **Spells tab** (caster-aware Browse button). This also fills A9's Reference section for spells. **Remaining:** equipment + inventory + real AC, item usage, class features + pool resources, spell prep/known + realistic slot tables, third-caster casting, hit-die sizing. (See the iOS README's "5e rules — coverage gaps".) *(iOS 10.)*
